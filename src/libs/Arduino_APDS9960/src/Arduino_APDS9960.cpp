...
APDS9960::APDS9960(TwoWire &wire, int intPin)
    : _wire(wire), _intPin(intPin), _gestureEnabled(false), _proximityEnabled(false), _colorEnabled(false),
      _gestureIn(false), _gestureDirectionX(0), _gestureDirectionY(0), _gestureDirInX(0), _gestureDirInY(0),
      _gestureSensitivity(20), _detectedGesture(GESTURE_NONE)
{
}
...
size_t APDS9960::readBlock(uint8_t reg, uint8_t *val, unsigned int len)
{
    ...
    if (!write(reg))
        return 0;
    _wire.requestFrom((int)APDS9960_ADDR, (int)len);
